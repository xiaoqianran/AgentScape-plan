...
### W-01 Formalize Constrained World IR Revision

**状态：IMPLEMENTED / MIGRATING TO INCREMENTAL EXECUTION — through `9c3a08b`.**

World revision 已按 strict World IR 重新实现：Runtime-owned base/next revision、Finding scope、affected entities 与 evidence linkage；Agent 仅提交 bounded typed edits，proposal 必须 fresh-round 后原样执行，stale/tampered proposal fail-closed。当前安全增量执行覆盖 `set-initial-state`、`set-capability-intent`、`set-physics-requirement` 与单实体/无关系参与的 `set-position`；无法证明 authority/scene 一致时自动 fallback full canonical rebuild。
...
**状态：FIRST CLOSED LOOP COMPLETE + MULTI-DOMAIN SAFE INCREMENTAL RECOMPILE — through `9c3a08b`.**

动作级验证与 world acceptance 已有强基础；`Finding v1` 已统一 Validator/Acceptance failure identity，并将 repair eligibility / strategy / affectedObjects / worldRevisionId 显式化；RepairEngine 对 stale revision fail-closed，非 repairable Finding 不会被擅自修复。`interaction-verified` 已不再读取 Planner/业务可写 state，而消费 revision-bound `InteractionEvidence`；对象替换/删除与 revision 变化都会使旧证据不可复用。Acceptance DSL 新增 Runtime-derived `relation-exists`（ON/NEAR/INSIDE/CONTAINS/SUPPORTS），`kind` 必填，避免 silent semantic erasure。Finding / Revision dependency closure 已纳入 relation subject/object 与 PLACE supportId。Affected-IR handoff 已升级为 Runtime-owned bounded proposal：Agent 只能提交 typed entity edits，不能自造 baseWorldIR/Finding scope/affectedEntityIds/revision identity；当前 edit 集为 `set-position / set-generation / replace-asset / set-initial-state / set-capability-intent / set-physics-requirement`。accepted proposal → recompile 现分为安全增量与 full canonical fallback：state-only、behavior-only、physics-only 与保守单-position slice 都先做 domain-specific preflight/admission，再 fresh Validator/Acceptance；无法证明 current Runtime 与 base revision 一致时自动 full rebuild。position path 复用 `WorldComposer` 的 bounds/footprint/Physics preflight，并排除 self collider；空间关系参与、多实体移动、held object 等场景不走增量。rejected/异常仍 restore scene 与 authority/evidence；verified recompile 只清理对应 base revision 的 unresolved mutation。当前 full suite `153 files / 707 tests PASS`，production build PASS。
...
AI-5  VER-01       Unified Finding + Acceptance Contract — FINDING + REPLAY MERGED (`8bf51ef` + `117912c`)
